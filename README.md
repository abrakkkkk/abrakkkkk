# Olá! Sou Abrahão Levy 👋

**Bacharel em Análise e Desenvolvimento de Sistemas — UNIFOR**

Profissional de TI em transição para a área de Dados, atuando na intersecção entre Engenharia e Análise de Dados. Meu objetivo é transformar grandes volumes de informação em decisões estratégicas.

<br>

## 🛠️ Stack de Tecnologias

**Dados & Analytics**
<br>
<img src="https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white" alt="PostgreSQL" />
<img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" alt="Python" />
<img src="https://img.shields.io/badge/Power_BI-F2C811?style=flat-square&logo=powerbi&logoColor=black" alt="Power BI" />
<img src="https://img.shields.io/badge/Streamlit-FF4B4B?style=flat-square&logo=streamlit&logoColor=white" alt="Streamlit" />
<img src="https://img.shields.io/badge/Pentaho-004B8D?style=flat-square&logo=pentaho&logoColor=white" alt="Pentaho" />

**Desenvolvimento**
<br>
<img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" alt="JavaScript" />
<img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white" alt="TypeScript" />
<img src="https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black" alt="React" />
<img src="https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white" alt="Node.js" />
<img src="https://img.shields.io/badge/SQL-CC0000?style=flat-square&logo=sqlite&logoColor=white" alt="SQL" />
<img src="https://img.shields.io/badge/Windows-0078D6?style=flat-square&logo=windows&logoColor=white" alt="Windows" />

<br>

## 🎯 Foco atual

- **Data Science** — modelos preditivos e análise estatística.
- **Modelagem de dados** — modelagem relacional e normalização (1FN, 2FN, 3FN).
- **ETL** — automação de pipelines com Pentaho e Python.
- **Segurança da informação** — estudos aplicados a dados e redes (Hackers do Bem).

<br>

## 🚀 Projetos

**[Dashboards de Business Intelligence](#)**  
Painéis para análise estratégica, integrando Power BI, Streamlit e PostgreSQL para visualização de dados em tempo real.

**[Sistema de RH (Modelagem de Dados)](#)**  
Desenvolvimento de diagramas entidade-relacionamento (MER/DER) e criação de tabelas estruturadas em SQL para o cadastro e gestão de funcionários.

**[Estudaê](#)**  
Aplicação mobile voltada para educação, desenvolvida durante a graduação.  
`React` · `TypeScript` · `Vite` · `Node.js` · `PostgreSQL`

<br>

## 📊 GitHub Stats

[![GitHub Stats](https://github-readme-stats.vercel.app/api?username=abrakkkkk&show_icons=true&theme=tokyonight&hide_border=true&locale=pt_BR&v=1)](https://github.com/abrakkkkk)

[![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=abrakkkkk&layout=compact&theme=tokyonight&hide_border=true&locale=pt_BR&v=1)](https://github.com/abrakkkkk)

<br>

## 📬 Contato

[LinkedIn](https://www.linkedin.com/in/abrahao-levy-04b85125b/) · [GitHub](https://github.com/abrakkkkk) · [Email](mailto:abrahaolevyy@gmail.com)
